...
exports.Message = function(time, author, content, channel, id, mentions, everybodyMentioned, embeds) {
    if (!content) {
        message = time;
        channel = author;
        ...
        everybodyMentioned = message.mention_everyone;
        embeds = message.embeds;
    }

    this.time = Date.parse(time);
    this.author = new User(author);
    this.content = content.replace(/\s+/g, ' ').trim();
    this.channel = channel;
    this.id = id;
    this.mentions = new List("id");
    this.everybodyMentioned = everybodyMentioned;
    this.embeds = embeds;
